-module(ridhm_pubsub_authorization).
...
ask_authentication(UserId, Channel, AuthConfig) ->
    case lists:keyfind(authorization_url, 1, AuthConfig) of
        false -> <<"bad value for authorization url in config">>;
        {authorization_url, AuthorizeUrl} ->
            case httpc:request(post, {AuthorizeUrl, [], "application/x-www-form-urlencoded", "userid="++binary_to_list(UserId)++"&channel="++binary_to_list(Channel)}, [], []) of
