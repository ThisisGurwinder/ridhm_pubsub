...
handle_call({publish, Message}, _From, State = #state{channel = Channel, user_id = UserId}) ->
    case already_authorized of
        true ->
            gen_server:cast(ridhm_pubsub_router, {publish, Message, channel, Channel}),
            {reply, ok, State};
        _ ->
...
can_publish(UserId, Channel) ->
    case application:get_env(ridhm_pubsub, publish_authorization) of
        undefined -> true;
        {ok, AuthConfig} ->
            ridhm_pubsub_authorization:check_authorization(UserId, Channel, AuthConfig)
end.